...
本项目实现 GB/T 28181-2022 标准协议，**该协议默认采用明文传输**（SIP 信令、RTP 媒体），UVP GB28181 Sim 目前的发布版本沿用了这一默认行为。

**推荐使用场景：**

- 局域网 / 同网段联调
- VPN / IPSec 等加密隧道内
- 受控测试环境（自建 WVP / 上级平台 IP 明确白名单）

**不建议的使用方式：**

- 直接把设备 / 模拟器暴露在公网（明文信令可被嗅探，媒体流可被截获，且 GB28181 Digest 认证使用 MD5）
- 跨广域网直连未知上级平台
- 同一设备复用多个互不信任的平台凭据

如发现安全问题，请按 [SECURITY.md](SECURITY.md) 流程报告，**勿提 public issue**。

**v1.1 路线图：** 计划评估 SIP over TLS + GB35114 国密扩展支持，为公网场景提供合规传输层选项。
